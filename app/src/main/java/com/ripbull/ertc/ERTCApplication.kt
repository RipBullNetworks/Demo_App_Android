 ...
 */
class ERTCApplication : MultiDexApplication(), LifecycleObserver {
  ...
  companion object{
    ...
    private val TAG = ERTCApplication::class.java.simpleName
  ...
  override fun onCreate() {
    ...
    if (session?.getAccessCode() != null) {
      ...
      val config = Configuration.Builder()
        .context(this)
        .apiKey(getString(R.string.access_token))
        .namespace("chat.dev.ertc.com").build()
      ...
      disposable.add(
        eRTCSDK.chat().messageOn()
          ...
          .filter { !((it.messageRecord().type == ChatType.SINGLE_CHAT_THREAD.type || it.messageRecord().type == ChatType.GROUP_CHAT_THREAD.type)
  ...
  fun onAppBackgrounded() {
    ...
    Timber.e(TAG, "************* backgrounded: $isAppInBackground")
    Timber.e(TAG, "************* ${isInForeground()}")
  }
  ...
  fun onAppForegrounded() {
    ...
    Timber.e(TAG, "************* foregrounded: $isAppInBackground")
    Timber.e(TAG, "************* ${isInForeground()}")
    clearNotifications(this)
  }

  private fun handleDataMessage(messageRecord: MessageRecord) {
    Timber.e(TAG, "handleDataMessage ***** ")
    ...
    try {
      ...
      if (!TextUtils.isEmpty(messageRecord.senderRecord?.name) && !TextUtils.isEmpty(body)
        ...
        title?.let {
          val parentMsgId = messageRecord.chatThreadMetadata?.parentMsgId
          showNotificationMessage(it, body, threadId!!, this, messageRecord.senderId, parentMsgId)
        }
      }
    } catch (e: Exception) {
      e.printStackTrace()

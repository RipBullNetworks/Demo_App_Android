...
class RoundedImageView : AppCompatImageView {
  ...
  override fun onSizeChanged(newWidth: Int, newHeight: Int, oldWidth: Int, oldHeight: Int) {
    ...
    val updatedNewWidth = newWidth - (_paddingStart + _paddingEnd)
    val updatedNewHeight = newHeight - (_paddingTop + _paddingBottom)

    if (pathWidth != updatedNewWidth || pathHeight != updatedNewHeight) {
      pathWidth = updatedNewWidth
      pathHeight = updatedNewHeight
      setupPath()
    }
  }
  ...
  private fun setupPath() {
    var isAllRound = false
    if (roundedTopLeft && roundedTopRight && roundedBottomRight && roundedBottomLeft) {
      isAllRound = true
    }
    if (isAllRound && (cornerRadius >= pathHeight / 2 && cornerRadius >= pathWidth / 2)) {
      isCircle = true
